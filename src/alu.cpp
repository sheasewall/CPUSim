...
std::bitset<32> ALU::add(std::bitset<32> val1, std::bitset<32> val2) {
  ...
  std::bitset<32> carries_shifted =
      hardwareLeftShift(carries, std::bitset<32>(1));
...
bool ALU::lessThanUnsigned(std::bitset<32> val1, std::bitset<32> val2) {
  ...
  std::bitset<32> highest_bit_val1 =
      hardwareRightShift(val1, std::bitset<32>(31));
  std::bitset<32> highest_bit_val2 =
      hardwareRightShift(val2, std::bitset<32>(31));
  ...
  if (hardwareIsEqual(high_bits_differ_and_val1_is_greater,
                      std::bitset<32>(1))) {
    return false;
  }
  ...
  if (hardwareIsEqual(high_bits_differ_and_val2_is_greater,
                      std::bitset<32>(1))) {
    return true;
  }
...
bool ALU::lessThanSigned(std::bitset<32> val1, std::bitset<32> val2) {
  ...
  if (hardwareIsEqual(high_bits_differ_and_val1_is_negative,
                      std::bitset<32>(1))) {
    return true;
  }
  ...
  std::bitset<32> high_bits_differ_and_val2_is_negative =
      bitwiseAnd(high_bits_differ, highest_bit_val2);
  if (hardwareIsEqual(high_bits_differ_and_val2_is_negative,
                      std::bitset<32>(1))) {
    return false;
  }
